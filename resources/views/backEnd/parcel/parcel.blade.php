                    ...
                    <table id="example" class="table table-bordered table-striped custom-table table-responsive" width="100%">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td><?php $area=App\Models\Deliverycharge::where('id',$value->orderType)->first(); ?> {{@$area->title}}</td>
                          <td>{{$value->recipientAddress}}</td>
                          <td>{{$value->recipientPhone}}</td>
                          <td>@if($value->deliverymanId) {{@$deliverymanInfo->name}} @else <button class="btn btn-primary" data-toggle="modal" data-target="#asignModal{{$value->id}}">Asign</button> @endif</td>
                          <!-- Modal -->
                                      ...
                                      <div id="sUpdateModal{{$value->id}}" class="modal fade" role="dialog">
                                        <div class="modal-dialog">
                                          ...
                                          <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                              <form action="{{url('editor/parcels/status-update')}}" method="POST">
                                                @csrf
                                                <input type="hidden" name="hidden_id" value="{{$value->id}}">
